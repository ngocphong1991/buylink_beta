...
function textlink_ads()
{
    ...
    $LOCA_VNL_FILENAME = "'.$script_code.'.xml";

    if (!file_exists($LOCA_VNL_FILENAME)) {
        @touch($LOCA_VNL_FILENAME);
        @chmod($LOCA_VNL_FILENAME, 0666);
    }

    if (!file_exists($LOCA_VNL_FILENAME)) {
        die("Script error: $LOCA_VNL_FILENAME does not exist. Please create a blank file named $LOCA_VNL_FILENAME.");
    }

    if (!is_writable($LOCA_VNL_FILENAME)) {
        die("Script error: $LOCA_VNL_FILENAME is not writable. Please set write permissions on $LOCA_VNL_FILENAME.");
    }
    ...
    if (filemtime($LOCA_VNL_FILENAME) < (time() - 3600) || filesize($LOCA_VNL_FILENAME) < 3) {
        ...
        textlink_updateLocal($url, $LOCA_VNL_FILENAME, $CONNECTION_TIMEOUT);
    }

    $xml = textlink_getLocal($LOCA_VNL_FILENAME);
...
function textlink_ads()
{
    ...
    // This file MUST be writable by web server
    // You should create a blank file and CHMOD it to 666
    $LOCAL_FILENAME = \''.$script_code.'.xml\';
